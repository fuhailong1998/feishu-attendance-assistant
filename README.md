...
一个运行在飞书网页版中的 Tampermonkey 用户脚本。它从「假勤」会话读取打卡消息，在浏览器本地生成每日考勤明细、异常汇总、有效工时和可切换的加班图表。
...
UI 测试覆盖响应式布局、键盘操作、跨会话缓存、自动定位、本地补充、封账周期、中英文消息解析，以及加班 K 线与趋势图切换。
